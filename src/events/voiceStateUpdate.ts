    ...
    try {
        ...
        for (const [userId, sessionData] of gracePeriodSessions.entries()) {
            ...
            if (gracePeriodElapsed >= GRACE_PERIOD_MS) {
                ...
                console.log(`⏰ Grace period expired for user ${userId} (${Math.floor(gracePeriodElapsed / 1000 / 60)} minutes)`);
                ...
                for (const [_guildId, guild] of discordClient.guilds.cache) {
                    try {
                        ...
                        console.warn(`Error checking voice state during grace period for user ${userId}:`, error.message);
                    }
                }
            }
        }
                    ...
                    try {
                        ...
                        if (voiceState?.channel) {
                            ...
                            if (sessionData.channelId !== voiceState.channel.id) {
                                console.log(`🔄 User ${userId} moved channels - updating tracking`);
                        ...
                        console.warn(`Error checking voice state for user ${userId} in guild ${guild.name}:`, error.message);
                    }
                }

                if (!userStillInVoice) {
                    const sessionAge = Math.floor((Date.now() - sessionData.joinTime.getTime()) / (1000 * 60));
        ...
        for (const { userId, sessionAge } of cleanupCandidates) {
            console.log(`🧹 Cleaning up abandoned session for user ${userId} (${sessionAge} minutes old, not in any voice channel)`);
            activeVoiceSessions.delete(userId);
            gracePeriodSessions.delete(userId); // Safety cleanup
        }

        if (cleanupCandidates.length > 0 || gracePeriodExpired > 0) {
            console.log(`✅ Enhanced cleanup completed: ${cleanupCandidates.length} abandoned sessions removed, ${gracePeriodExpired} grace periods expired`);
            console.log(`📊 Cleanup stats: ${sessionsChecked} active sessions checked, ${longSessions} long sessions verified, ${gracePeriodSessions.size} in grace period`);
        } else if (longSessions > 0 || gracePeriodSessions.size > 0) {
            console.log(`✅ Enhanced cleanup completed: All ${longSessions} long sessions are legitimate, ${gracePeriodSessions.size} users in grace period`);
        }
...
setInterval(async() => {
    ...
    console.log(`🌅 Checking for midnight crossover sessions (${activeVoiceSessions.size} active sessions)...`);
        ...
        for (const [userId, sessionData] of activeVoiceSessions.entries()) {
            try {
                ...
                if (sessionStartDay !== today) {
                    ...
                    const result = await voiceService.handleMidnightCrossover(userId, sessionData.channelId, member);
                ...
                console.error(`Error handling midnight crossover for user ${userId}:`, error);
            }
        }

        if (crossoversHandled > 0) {
            console.log(`✅ Midnight crossover completed: ${crossoversHandled} sessions processed`);
        }
...
async function manualCleanup() {
    ...
    for (const [userId, sessionData] of gracePeriodSessions.entries()) {
        ...
        if (gracePeriodElapsed >= GRACE_PERIOD_MS) {
            console.log(`🧹 Manual cleanup: Grace period expired for user ${userId} (${Math.floor(gracePeriodElapsed / 1000 / 60)} minutes)`);
    ...
    for (const { userId, sessionAge } of cleanupCandidates) {
        console.log(`🧹 Manual cleanup: Removing abandoned session for user ${userId} (${sessionAge} minutes old)`);
        activeVoiceSessions.delete(userId);
        gracePeriodSessions.delete(userId); // Safety cleanup
        results.cleanedUp++;
    }

    console.log(`✅ Manual cleanup completed: ${results.cleanedUp}/${results.checkedSessions} sessions cleaned up, ${results.gracePeriodExpired} grace periods expired`);
    return results;
}
        ...
        try {
            ...
            if (!oldChannel && newChannel) {
                ...
                if (gracePeriodSessions.has(userId)) {
                    console.log(`🔄 ${username} returned during grace period to: ${newChannel.name}`);
                    ...
                    console.log(`✅ Session resumed for ${username} - no interruption to voice tracking`);
                } else {
                ...
                if (sessionData) {
                    ...
                    console.log(`⏸️ ${username} left voice channel: ${oldChannel.name} - starting 5-minute grace period`);

                    sessionData.gracePeriodStart = Date.now();
                    gracePeriodSessions.set(userId, sessionData);

                    console.log(`🕐 Grace period started for ${username} - session will end if not back within 5 minutes`);
                } else {
