...
module.exports = {
    ...
    async execute(interaction) {
        try {
            ...
            const totalGracePeriod = gracePeriodSessions ? gracePeriodSessions.size : 0;

            const statsCard = createStatsCard('Session Overview', {
                'Active Sessions': `${totalActive}`,
                'Grace Period': `${totalGracePeriod}`,
                'Total Tracked': `${totalActive + totalGracePeriod}`,
                'Grace Period %': totalActive + totalGracePeriod > 0
            ...
            embed.setDescription(`Monitor users experiencing connection issues\n\n${statsCard}`);
            ...
            if (gracePeriodSessions && gracePeriodSessions.size > 0) {
                ...
                for (const [userId, sessionData] of gracePeriodSessions.entries()) {
                    try {
                        ...
                        const gracePeriodRemaining = Math.max(0, Math.floor((GRACE_PERIOD_MS - gracePeriodElapsed) / (1000 * 60)));
                        ...
                        try {
                            ...
                            username = user.username.length > 12 ? user.username.substring(0, 9) + '...' : user.username;
                        } catch (error) {
                        ...
                        console.error(`Error processing grace period session ${userId}:`, error);
                    }
                }

                if (gracePeriodData.length > 0) {
                    const gracePeriodTable = formatDataTable(gracePeriodData, [14, 10, 12]);

                    embed.addFields([
                        {
                            name: createHeader('Users in Grace Period', null, '⏳', 'emphasis'),
                ...
                embed.addFields([
                    {
                        ...
                        value: 'No users currently in grace period - all connections stable!',
                        inline: false
                    }
                ]);
            }
            ...
            if (activeVoiceSessions.size > 0) {
                const longSessions = Array.from(activeVoiceSessions.values())
                    .filter(session => !gracePeriodSessions || !gracePeriodSessions.has(Array.from(activeVoiceSessions.keys()).find(key => activeVoiceSessions.get(key) === session)))
            ...
            embed.addFields([
                {
                    ...
                    value: '**Purpose:** Prevents session loss for users with unstable connections • **Duration:** 5 minutes maximum • **Behavior:** Voice tracking continues during brief disconnections • **Benefits:** Maintains session continuity and 55-minute rounding accuracy',
                    inline: false
                }
            ]);
            ...
            const errorMessage = '❌ Grace period check failed. Please try again later.';
            try {
