...
class PerformanceMonitor {
  ...
  trackCommand(commandName, startTime, endTime, error = null) {
    ...
    commandMetrics.avgResponseTime =
      commandMetrics.totalTime / commandMetrics.totalExecutions;
  ...
  getPerformanceSummary() {
    ...
    const memoryTrend =
      recentMemory.length > 1
        ? recentMemory[recentMemory.length - 1].heapUsed -
    ...
    return {
      ...
      commands: {
        total: Array.from(this.metrics.commands.values()).reduce(
      ...
      database: {
        totalQueries: Array.from(this.metrics.database.values()).reduce(
          (sum, db) => sum + db.count,
          0,
        ),
  ...
  identifyBottlenecks() {
    ...
    if (summary.memory.current.heapUsed > 200) {
      // 200MB is more realistic for monitoring
      issues.push({
        type: "memory",
    ...
    if (summary.memory.trend > 20) {
      // 20MB increase per minute is concerning
      issues.push({
        type: "memory_leak",
        severity: "critical",
    ...
    if (summary.memory.current.rss > 300) {
      // 300MB RSS is getting high for Discord bot
      issues.push({
        type: "rss_memory",
