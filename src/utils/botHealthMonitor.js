...
class BotHealthMonitor {
    ...
    setupHealthChecks() {
        ...
        this.healthChecker.registerCheck('memory', async () => {
            ...
            // Use heap usage for health check instead of system memory
            if (heapUsagePercent > 95) {
                throw new Error(`Critical heap usage: ${heapUsagePercent.toFixed(1)}%`);
            }
            
            // Also check for excessive RSS usage (should be reasonable for a Discord bot)
            if (rssMB > 1000) { // 1GB RSS is quite high for a Discord bot
                throw new Error(`High RSS memory usage: ${rssMB}MB`);
            }
            ...
            return {
                heap: {
                    ...
                    status: heapUsagePercent > 90 ? 'high' : heapUsagePercent > 80 ? 'moderate' : 'normal'
                },
                process: {
                    rss: rssMB,
                    external: externalMB,
