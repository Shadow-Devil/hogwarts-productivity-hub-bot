...
class DailyTaskManager {
  ...
  async checkMidnightCleanup() {
    ...
    if (now.hour() === 0 && now.minute() === 0) {
      console.log(
        "🌅 Starting midnight cleanup and daily voice stats reset...",
      );
  ...
  async handleActiveVoiceSessionsAtMidnight() {
    try {
      ...
      console.log(
        "✅ All voice session processing completed during daily reset",
      );
      return resetResults;
    } catch (error) {
      console.error(
  ...
  async resetDailyVoiceStats() {
    return executeWithResilience(async (client) => {
      ...
      for (const userId of allActiveUsers) {
        try {
          const result = await this.processUserMidnightReset(
            userId,
            activeVoiceSessions,
            gracePeriodSessions,
            voiceService,
   ...
   */
  async processUserMidnightReset(
    userId,
    activeVoiceSessions,
    gracePeriodSessions,
    voiceService,
    ...
    return measureDatabase("getUserDailyTaskStats", async () => {
      return executeWithResilience(async (client) => {
        ...
        const remaining = Math.max(
          0,
          this.DAILY_TASK_LIMIT - stats.total_task_actions,
        );
    ...
    return measureDatabase("reclaimTaskSlot", async () => {
      return executeWithResilience(async (client) => {
        ...
        const currentAvailableSlots =
          this.DAILY_TASK_LIMIT - currentTotalActions;
