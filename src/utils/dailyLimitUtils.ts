...
function calculateDailyLimitInfo(dailyHours, dailyLimitHours = 15, userTimezone = null) {
    ...
    const remainingHours = Math.max(0, Math.min(allowanceHoursRemaining, hoursUntilMidnight));
 ...
 */
function generateDailyLimitMessage(limitInfo, actualSessionHours, voiceTimePoints) {
    if (!limitInfo.limitReached) {
