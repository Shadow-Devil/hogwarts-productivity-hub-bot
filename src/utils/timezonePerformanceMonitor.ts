...
class TimezonePerformanceMonitor {
  ...
  recordCacheOperation(operation, metadata = {}) {
    ...
    const total =
      this.metrics.cacheOperations.hits + this.metrics.cacheOperations.misses;
    const hitRate =
      total > 0 ? (this.metrics.cacheOperations.hits / total) * 100 : 0;
  ...
  getMetrics() {
    ...
    // Calculate averages
    const avgTimezoneConversion =
      this.metrics.timezoneConversions.count > 0
        ? this.metrics.timezoneConversions.totalDuration /
          this.metrics.timezoneConversions.count
        : 0;

    const avgDatabaseQuery =
      this.metrics.databaseQueries.count > 0
        ? this.metrics.databaseQueries.totalDuration /
          this.metrics.databaseQueries.count
        : 0;
  ...
  reportMetrics() {
    ...
    if (
      metrics.cacheOperations.hitRate < 80 &&
      metrics.cacheOperations.hits + metrics.cacheOperations.misses > 100
    ) {
