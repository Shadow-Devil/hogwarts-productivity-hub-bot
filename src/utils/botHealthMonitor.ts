...
class BotHealthMonitor {
      ...
      async () => {
        ...
        if (commandStats.length > 0) {
          ...
          if (errorRate > this.alertThresholds.errorRatePercent) {
            throw new Error(
              `High command error rate: ${errorRate.toFixed(1)}%`,
            );
          }
          ...
          return {
            ...
            slowCommands: commandStats.filter(
              (cmd) => cmd.avgResponseTime > 2000,
            ).length,
          };
        }
  ...
  logHealthSummary() {
    ...
    console.log(
      `   Status: ${this.getStatusEmoji(overall.status)} ${overall.status.toUpperCase()}`,
    );
    ...
    if (perfResult && perfResult.status === "healthy") {
      ...
      console.log(
        `   Commands: ${Object.keys(details.commands).length} active`,
      );
      console.log(
