...
let client: Client; // Discord client instance
let databaseResilience: any;
...
let healthCheckInterval: NodeJS.Timeout | null;
let healthSummaryInterval: NodeJS.Timeout | null;

export function initialize(client_, databaseResilience_) {
  client = client_;
  databaseResilience = databaseResilience_;
  healthChecker;
  healthHistory;
  maxHistorySize;
  alertThresholds;
...
function setupHealthChecks() {
  ...
  healthChecker.registerCheck(
    "database",
    async () => {
      return await databaseResilience.healthCheck();
    },
  ...
  healthChecker.registerCheck(
    ...
    async () => {
      ...
      if (capacityPercent > 85) {
        throw new Error(
          `Critical V8 heap capacity: ${capacityPercent.toFixed(1)}% of ${maxHeapMB}MB limit`
        );
      }
      ...
      if (rssMB > 500) {
        throw new Error(
          `High RSS memory usage: ${rssMB}MB (consider process restart)`
        );
      }
      ...
      if (heapUsagePercent > 90) {
        console.log(
          `🟡 Memory efficiency: ${efficiency} - Heap ${heapUsagePercent.toFixed(1)}% full (${heapUsedMB}/${heapTotalMB}MB)`
        );
      }

      if (capacityPercent > 60) {
        console.log(
          `📊 V8 capacity usage: ${capacityPercent.toFixed(1)}% of ${maxHeapMB}MB limit`
        );
      }
  ...
  healthChecker.registerCheck(
    ...
    async () => {
      ...
      if (criticalIssues.length > 0) {
        throw new Error(
          `Critical performance issues: ${criticalIssues.map((i) => i.message).join(", ")}`
        );
      }
    ...
    async () => {
      ...
      if (commandStats.length > 0) {
        const avgResponseTime =
          commandStats.reduce((sum, cmd) => sum + cmd.avgResponseTime, 0) /
          commandStats.length;

        if (avgResponseTime > alertThresholds.responseTimeMs) {
          throw new Error(
            `Slow command responses: ${avgResponseTime.toFixed(0)}ms average`
          );
        }
        ...
        const totalErrors = commandStats.reduce(
          (sum, cmd) => sum + cmd.errors,
          0
        );
        const errorRate =
          totalCommands > 0 ? (totalErrors / totalCommands) * 100 : 0;
        ...
        return {
          ...
          slowCommands: commandStats.filter((cmd) => cmd.avgResponseTime > 2000)
            .length,
        };
      }
...
function startPeriodicChecks() {
  ...
  healthSummaryInterval = setInterval(() => {
    logHealthSummary();
  }, 300000);
}
...
export async function runHealthCheck() {
  ...
  const healthSnapshot = {
    ...
    duration,
    results,
    overall: healthChecker.getOverallHealth(),
  };

  // Store in history
  healthHistory.push(healthSnapshot);
  if (healthHistory.length > maxHistorySize) {
    healthHistory.shift();
  }

  // Check for critical issues
  await handleHealthResults(healthSnapshot);

  return healthSnapshot;
}

export async function handleHealthResults(healthSnapshot) {
  ...
  if (overall.status === "critical") {
    ...
    await attemptAutoRecovery(overall.issues);
...
export async function attemptAutoRecovery(issues) {
  for (const issue of issues) {
    try {
      switch (issue.name) {
        case "database":
          console.log("🔄 Attempting database recovery...");
          databaseResilience.resetCircuitBreakers();
          break;
        ...
        case "discord_client":
          console.log(
            "🔌 Discord client issue detected - monitoring for auto-reconnect..."
          );
      ...
      console.error(
        `❌ Auto-recovery failed for ${issue.name}:`,
        recoveryError
      );
    }
  }
...
export function logHealthSummary() {
  ...
  console.log(
    `   Status: ${getStatusEmoji(overall.status)} ${overall.status.toUpperCase()}`
  );
  ...
  if (perfResult && perfResult.status === "healthy") {
    ...
    console.log(
      `   Database: ${details.database.totalQueries} queries, ${details.database.activeConnections} connections`
    );
  }
}
...
export function getHealthReport(): HealthReport {
  ...
  const latest = healthHistory[healthHistory.length - 1];
  const last10 = healthHistory.slice(-10);

  // Calculate uptime percentage for last 10 checks
  const healthyChecks = last10.filter(
    (h) => h.overall.status === "healthy"
  ).length;
...
export function calculateTrends() {
  ...
  if (memoryValues.length >= 2) {
    ...
    trends.memory =
      memTrend > 5 ? "increasing" : memTrend < -5 ? "decreasing" : "stable";
  }
  ...
  const responseValues = recent
    .filter(
      (h) =>
        h.results.command_responsiveness &&
        h.results.command_responsiveness.status === "healthy"
    )
    .map((h) => h.results.command_responsiveness.details.averageResponseTime);

  if (responseValues.length >= 2) {
    const respTrend =
      responseValues[responseValues.length - 1] - responseValues[0];
    trends.responseTime =
      respTrend > 100 ? "slowing" : respTrend < -100 ? "improving" : "stable";
  }
...
export function shutdown() {
  ...
  if (healthSummaryInterval) {
    clearInterval(healthSummaryInterval);
  }
