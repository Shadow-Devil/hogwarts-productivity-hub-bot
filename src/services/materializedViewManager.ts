...
class MaterializedViewManager {
  ...
  startAutoRefresh(intervalMinutes = 5) {
    ...
    console.log(
      `🔄 Starting materialized view auto-refresh every ${intervalMinutes} minutes`,
    );
  ...
  async refreshViews() {
    if (this.isRefreshing) {
      console.log(
        "⏳ Materialized view refresh already in progress, skipping...",
      );
      return false;
    }
  ...
  invalidateRelatedCaches() {
    try {
      ...
      console.log(
        "🧹 Related caches invalidated after materialized view refresh",
      );
    } catch (error) {
      console.warn(
        "⚠️ Warning: Failed to invalidate some caches after refresh:",
        error.message,
      );
    }
  }
  ...
  needsRefresh(maxAgeMinutes = 10) {
    ...
    const ageMinutes =
      (Date.now() - this.lastRefreshTime.getTime()) / (1000 * 60);
  ...
  async refreshIfStale(maxAgeMinutes = 10) {
    if (this.needsRefresh(maxAgeMinutes)) {
      console.log(
        `🔄 Materialized views are stale (>${maxAgeMinutes} minutes), refreshing...`,
      );
    ...
    try {
      ...
      const failureRate =
        stats.totalRefreshes > 0
          ? (stats.failedRefreshes / stats.totalRefreshes) * 100
          : 0;

      if (failureRate > 20) {
        health.status = "warning";
        health.issues.push(
          `High refresh failure rate: ${failureRate.toFixed(1)}%`,
        );
        health.recommendations.push(
          "Check database connectivity and view definitions",
        );
      }
      ...
      if (!stats.autoRefreshActive) {
        ...
        health.recommendations.push(
          "Start auto-refresh for optimal performance",
        );
      }
