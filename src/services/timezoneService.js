...
class TimezoneService extends BaseService {
    ...
    async getCurrentTimeInUserTimezone(userId) {
        ...
        try {
            ...
            this.logger.error('getCurrentTimeInUserTimezone failed, falling back to UTC', {
                userId,
                error: error.message,
                stack: error.stack,
                duration: Date.now() - startTime,
    ...
    async updateLastResetTime(userId, resetType) {
        try {
            ...
            this.logger.warn('Could not update reset time', { userId, resetType, error: error.message });
            return false;
        }
    }
