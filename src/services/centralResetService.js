...
class CentralResetService extends BaseService {
    constructor() {
        ...
        this.logger.info('CentralResetService initialized', {
            features: ['timezone-aware resets', 'cron scheduling', 'structured logging'],
            logLevel: 'info'
        });
    }
    ...
    async processDailyResets() {
        ...
        try {
            ...
            const usersNeedingReset = await timezoneService.getUsersNeedingDailyReset();
    ...
    async updateUserStreak(user) {
        try {
            ...
            if (!hadActivityYesterday) {
                ...
                this.logger.debug('Streak preserved for user (had activity yesterday)', {
                    userId: user.discord_id,
                    timezone: user.timezone
                });
            }
    ...
    async processMonthlyResets() {
        ...
        try {
            ...
            const usersNeedingReset = await timezoneService.getUsersNeedingMonthlyReset();
    ...
    getServiceStatus() {
        return {
            ...
            activeJobs: Array.from(this.scheduledJobs.keys()),
            resetStats: this.resetStats,
            uptime: this.isRunning ? Date.now() - (this.resetStats.dailyResets.lastRun?.getTime() || Date.now()) : 0
        };
    }
